...
class Surrogate(object):
    def __init__(self, name, x_train, x_test, y_train, y_test, ratio, path, dropout):
        ...
        self.results_path = './Results/results_10000_sgd_no_trf_and_combine/results_'+name+'_10000_'+str(self.batch_ratio)+'_'+str(self.dropout)
    ...
    def train_model(self,):
        ...
        with open(self.results_path+'/results.txt', 'w') as file:
            for index in range(self.num_batches):
                ...
                self.krnn.compile(loss='mse', optimizer=sgd, metrics=['mse'])
                x_train = self.x_train_batches[index]
                y_train = self.y_train_batches[index]
                # for jindex in range(0, index):
                #     x_train = np.vstack([x_train, self.x_train_batches[jindex]])
                #     try:
                #         y_train = np.concatenate((y_train, self.y_train_batches[jindex]), axis=0)
                #     except Exception as e:
                #         print(y_train.shape, self.y_train_batches[jindex].shape)
                #         raise(e)

                train_log = self.krnn.fit(x_train, y_train, batch_size=10, epochs=100, validation_split=0.1, verbose=2, callbacks=[early_stopping])
                ...
                model_signature += 1
            file.write('Time taken: {}'.format(time.time() - start))
...
if __name__ == '__main__':
    ...
    # Parameters for data
    problem_paths = {'etopo':'etopo_fast_10000/surrogate/prediction_benchmark_data', 'crater':'crater_10000/surrogate/prediction_benchmark_data', 'mountain':'mountain_10000/surrogate/prediction_benchmark_data'}
    ...
    for ratio in batch_ratios:
        for dropout in dropout_values[1:2]:
            for problem in problems:
                ...
                ################################
                print problem, ':  train: ', x_train.shape[0], ' test: ', x_test.shape[0]
