    ...
    def run(self):
        ...
        for i in range(samples-1):
            ...
            '''if ( (i+1) % swap_int  == 0 ):
                ...
                print(' self.parameter_queue.get  ')
...
class ParallelTempering:
    ...
    def __init__(self,  vec_parameters,  minlimits_vec, maxlimits_vec, stepratio_vec,  inittopo_expertknow, rain_region, rain_time,  len_grid,  wid_grid, num_chains, maxtemp,NumSample,swap_interval, fname, 
        ...
        self.surrogate_pause_events = [multiprocessing.Event() for i in range(self.num_chains)]
    ...
    def initialize_chains (self,      check_likelihood_sed,   burn_in):
        ...
        for i in range(0, self.num_chains):
            ...
            self.surrogate_prob,self.surrogate_pause_events[i],self.surrogate_resume_events[i], 
    ...
    def run_chains(self):
        ...
        for j in range(0,self.num_chains):
            ...
            self.surrogate_resume_events[j].clear()
            self.surrogate_pause_events[j].clear()
            self.chains[j].start()
        ...
        swaps_appected_main =0
        total_swaps_main =0
        '''for i in range(int(self.NumSamples/swap_int)):
            print(i, ' swap check')
            count = 0
        ...
        for i in range(int(self.NumSamples/self.swap_interval)):
            ...
            for index in range(self.num_chains):
                if not self.chains[index].is_alive():
                    count+=1
                    self.surrogate_pause_events[index].set()
                    print(str(self.chains[index].temperature) +" Dead")
                else:
            ...
            for index in range(0,self.num_chains):
                ...
                flag = self.surrogate_pause_events[index].wait()
                if flag:
                    print("Signal from chain: {}".format(index+1))
                    # self.pause_chain_events[index].clear()
                    signal_count += 1
            ...
            if signal_count == self.num_chains:
                ...
                for index in range(self.num_chains):
                    ...
                    self.surrogate_pause_events[index].clear()
