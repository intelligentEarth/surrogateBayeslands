    ...
    def predict(self, X_load, initialized, model_signature):
        ...
        if self.model_id == 3:
            ...
            if initialized == False:
                ...
                self.model_signature = model_sign -1  # model_signature model_sign
                while True:
                    try:
...
class ptReplica(multiprocessing.Process):
    ...
    def run_badlands(self, input_vector):
        ...
        for x in range(len(self.sim_interval)):
            ...
            elev_vec[self.simtime] = self.real_elev
            erodep_vec[self.simtime] = self.real_elev
            erodep_pts_vec[self.simtime] = self.real_erodep_pts
    ...
    def run(self):
        ...
        for i in range(samples-1):
            ...
            if (i%self.surrogate_interval == 0) and (i!=0): 
                ...
                print(local_model_signature, ' local_model_signature -----------------------------------------------------------')
                ...
                np.savetxt(self.folder+'/surrogate/traindata_'+ str(int(self.temperature*10)) +'_'+str(self.model_signature)    +'_.txt', surr_train_set)
