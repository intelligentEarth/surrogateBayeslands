...
print(args.samples,' is samples args')
...
class ptReplica(multiprocessing.Process):
    def __init__(self,   num_param, vec_parameters,  inittopo_expertknow, rain_region, rain_time, len_grid, wid_grid, minlimits_vec, maxlimits_vec,
     ...
     xmlinput,  run_nb, tempr, parameter_queue,event , main_proc,   burn_in, surrogate_parameterqueue, surrogate_interval,surrogate_prob, surrogate_start,
        ...
        self.surrogate_pause_chain_event = surrogate_start
        self.surrogate_resume_chain_event = surrogate_resume
    ...
    def run(self):
        ...
        surr_train_set = np.zeros((100, self.num_param+1))
        ...
        for i in range(samples-1):
            ...
            print(self.surrogate_interval,self.swap_interval, 'are intervals')
            if i%self.surrogate_interval==0 and i!=0:
                print("at surrogate interval ",i)
                surr_train = surr_train_set[1:count_real, :]
                print( surr_train.shape,  self.temperature, ' surr_train_set.shape')
                self.surrogate_parameter_queue.put(surr_train)
                print(self.surrogate_parameter_queue,self.surrogate_parameter_queue.qsize(),self.temperature)
                self.surrogate_pause_chain_event.set()
                self.surrogate_resume_chain_event.clear()
                self.surrogate_resume_chain_event.wait()
            ...
            if i%self.swap_interval == 0 and i != 0:
                ...
                #all_param = np.random.rand(5,self.num_param+1)
        ...
        self.surrogate_pause_chain_event.set()
        self.pause_chain_event.set()
        print('chain ended ',self.temperature)
class ParallelTempering:
    def __init__(self,  vec_parameters,  minlimits_vec, maxlimits_vec, stepratio_vec,  inittopo_expertknow, rain_region, rain_time,  len_grid,  wid_grid, num_chains, maxtemp,NumSample,swap_interval, fname, 
        ...
        self.parameter_queue = [manager.Queue() for i in range(num_chains)]
        ...
        self.surrogate_interval = surrogate_interval
        self.surrogate_prob = surrogate_prob
        self.surrogate_pause_chain_events = [multiprocessing.Event() for i in range(self.num_chains)]
        self.surrogate_resume_chain_events = [multiprocessing.Event() for i in range(self.num_chains)]
        self.surrogate_parameter_queues = [manager.Queue() for i in range(self.num_chains)]
        ...
        self.inittopo_expertknow =  inittopo_expertknow 
        self.lock = [multiprocessing.Event() for i in range (2)] # for runmcmc and runsurrogate
        self.events = [multiprocessing.Event() for i in range (self.num_chains)] # to check if chains are alive or not
        ...
        for i in range(0, self.num_chains):
            self.chains.append(ptReplica(  self.num_param, self.vec_parameters, self.inittopo_expertknow, self.rain_region, self.rain_time, self.len_grid, self.wid_grid,
             self.minlimits_vec, self.maxlimits_vec, self.stepratio_vec,  check_likelihood_sed ,self.swap_interval, self.sim_interval,   self.simtime, self.NumSamples, self.real_elev, 
               self.real_erodep_pts, self.erodep_coords, self.folder, self.xmlinput,  self.run_nb,self.temperatures[i], self.parameter_queue[i],self.events[i], self.wait_chain[i],burn_in, 
               self.surrogate_parameter_queues[i],self.surrogate_interval,
            self.surrogate_prob,self.surrogate_pause_chain_events[i],self.surrogate_resume_chain_events[i], 
    ...
    def runSurrogate(self):
        ...
        while True:
            ...
            for index in range(self.num_chains):
                ...
                if not self.events[index].wait(timeout=1):
                    count+=1
                    # self.pause_chain_events[index].set()
            if count == self.num_chains:
                break
            if count>0:
                print("not all dead from run surrogate ",count)

            print("Waiting for surrogate signal.")
            # Check for signal from individual chains for swap
            signal_count = 0
            for index in range(self.num_chains):
                print('waiting for chain surrogate ',index)
                # print("Waiting for chain: {}. Chain alive: {}".format(index+1, self.chains[index].is_alive()))
                flag = self.surrogate_pause_chain_events[index].wait()
                if flag:
                    # print("Signal from chain: {}".format(index+1))
                    # self.pause_chain_events[index].clear()
                    signal_count += 1
            if not self.events[0].wait(timeout=1):
                continue
            ...
            if signal_count == self.num_chains:
                ...
                for index in range(0,self.num_chains):
                    print('starting surr',index)
                    queue_surr =  self.surrogate_parameter_queues[index]
                    print(queue_surr,queue_surr.qsize(),index)
                    # print('length of queue surr',len(queue_surr))
                    surr_data = queue_surr.get()
                    print(surr_data.shape, index, ' surr params')
                    all_param =   np.concatenate([all_param,surr_data],axis=0) 
                    print(all_param.shape, index, ' all_param')
                data_train = all_param[1:,:]  
                print(data_train.shape,   ' data_train  ---  ** -------------------------------')
                self.surrogate_trainer(data_train)
            elif signal_count != 10:
                print("kya karu jab kismat me yehi likha tha")
            for index in range(self.num_chains):
                self.surrogate_resume_chain_events[index].set()
                self.surrogate_pause_chain_events[index].clear()
        # self.lock[0].set()
        return

    def runMcmc(self,arr):
        print( '\n\nrunning runMCMC\n\n')
        ...
        # for i in range(int(self.NumSamples/self.swap_interval)):
            ...
            if count == self.num_chains:
                break
            if count>0:
                print("not all dead ",count)

            print("Waiting for swap signal.")
            # Check for signal from individual chains for swap
            signal_count = 0
            for index in range(0,self.num_chains):
                print('waiting for chain mcmc',index)
                # print("Waiting for chain: {}. Chain alive: {}".format(index+1, self.chains[index].is_alive()))
            ...
            for index in range(self.num_chains):
                ...
                self.pause_chain_events[index].clear()
        arr[0] = swaps_appected_main
        arr[1] = total_swaps_main
        # self.lock[1].set()
        return
    ...
    def run_chains(self):
        ...
        for j in range(0,self.num_chains):
            ...
            self.surrogate_pause_chain_events[j].clear()
            self.surrogate_resume_chain_events[j].clear()
            self.chains[j].start()
            self.events[j].set()
        swaps_appected_main = 0
        total_swaps_main = 0
        ...
        # self.lock[1].clear()
        a = [0,0]
        k = multiprocessing.Process(target=self.runSurrogate)
        k.start()
        p = multiprocessing.Process(target=self.runMcmc,args=(a,))
        p.start()
        print("\n\nboth running\n\n")
        # now to see if they ended or not
        # self.lock[0].wait()
        # self.lock[1].wait()
        p.join()
        k.join()
        print('retrieved both chains')
        swaps_appected_main = a[0]
        total_swaps_main = a[1]
        for j in range(0,self.num_chains):
            self.chains[j].join()
...
def main():
    ...
    # surrogate_interval = int(swap_interval/2)
    swap_interval = 2
